 ...
 * @returns {Promise<{ decoded: import("./types").CpuProfile, sourceMisses: Set<string> }>}
 */
export async function decodeCpuProfile(profilejson, sourcemapjsons) {
  ...
  const decoded = {
    ...
    nodes: profilejson.nodes.map((node) => {
      ...
      if (!consumer) {
        sourceMisses.add(callFrame.url);
        return node;
      }
